...
import axios from 'axios' // 1. ADICIONADO
import {
    LayoutDashboard, Building2, Settings, Power, Search, Bell, ShieldCheck, Menu, X, User, Loader2
} from 'lucide-react'
import { toast } from 'sonner'
import { authService } from '../../services/auth'
import ConfirmLogoutModal from '../../pages/admin/components/modal_confirmLogout'

const API_URL = import.meta.env.VITE_API_URL // 2. API URL
...
const getToken = (): string | null => localStorage.getItem('access_token'); // 3. PEGAR TOKEN
...
export default function MainLayout() {
    ...
    const [searchResults, setSearchResults] = useState<SearchResult[]>([]) // 4. ESTADO REAL
    const [searching, setSearching] = useState(false) // 5. LOADING
    ...
    // 6. BUSCA REAL NO DB COM DEBOUNCE
    useEffect(() => {
        if (!searchQuery.trim()) {
            setSearchResults([])
            return
        ...
        const delay = setTimeout(async () => {
            setSearching(true)
            try {
                // Rota única que retorna tudo: escolas, usuarios, definicoes
        ...
        }, 400) // espera 400ms pra não bombardear a API
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5 h-full flex flex-col shadow-2xl shadow-black/20">
                    <div className="flex items-center justify-between mb-10 px-1">
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-xs bg-[#3B82F6]/20 text-[#3B82F6] px-2.5 py-1 rounded-md font-bold flex-shrink-0">Admin</span>
                        </div>
                    ...
                    <nav className="space-y-2 flex-1">
                        {menuItems.map(item => {
                            ...
                            return (
                                <button key={item.path} onClick={() => handleNavigate(item.path)} className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl text-[15px] transition ${isActive? 'bg-[#3B82F6]/20 text-[#3B82F6] font-semibold border border-[#3B82F6]/30' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                    ...
                    <div className="border-t border-white/10 pt-5 mt-5">
                        <div className="flex items-center gap-4 px-1">
                            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#3B82F6] to-[#8B5CF6] flex items-center justify-center flex-shrink-0"><User className="w-6 h-6 text-white" /></div>
                            <div className="min-w-0 flex-1">
                                <p className="text-[15px] font-bold text-white truncate">{user.nome}</p>
                ...
                <header className="fixed top-0 right-0 left-0 lg:left-[260px] z-30 p-4 lg:p-8">
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl px-4 lg:px-6 py-3 lg:py-4 flex items-center justify-between gap-3 shadow-lg shadow-black/10">
                        ...
                        <div className={`relative flex-1 transition-all duration-300 ${isSearchOpen? 'max-w-[500px] opacity-100' : 'max-w-0 opacity-0'} hidden md:block`}>
                            ...
                            {searchQuery && (
                                <div className="absolute top-14 w-full bg-[#1E293B]/90 backdrop-blur-xl border-white/10 rounded-xl shadow-2xl max-h-80 overflow-y-auto z-50">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button className="p-2.5 lg:p-3 bg-white/5 border-white/10 rounded-xl hover:bg-white/10 transition"><Bell className="w-5 h-5 text-white" /></button>
                            ...
                            <button onClick={() => setLogoutOpen(true)} className="p-2.5 lg:p-3 bg-red-500/10 border-red-500/20 rounded-xl hover:bg-red-500/20 hover:border-red-500/40 transition group" title="Sair">
