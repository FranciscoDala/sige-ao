            ...
            <div ref={dropdownRef} className="relative">
              ...
              >
                <span className="flex items-center gap-3 truncate">
                  ...
                  {selectedEscola?.nome || (loadingEscolas ? "Carregando escolas..." : "Selecione sua escola")}
                </span>
                <ChevronDown className={`w-5 h-5 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`}/>
              </button>

              {dropdownOpen && (
                // AQUI: SELECT SÓLIDO SEM VIDRO
                <div className="absolute z-10 w-full mt-2 bg-[#1A1A1A] border border-white/20 rounded-xl shadow-2xl overflow-hidden">
                  <div className="max-h-60 overflow-y-auto">
                    {escolas.length === 0 && <div className="p-3 text-center text-white/50 text-sm">Nenhuma escola encontrada</div>}
                    {escolas.map(e => (
                      <button
                        key={e.id}
                        type="button"
