...
export default function MainLayout() {
    ...
    return (
        <div className="min-h-screen w-full bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#0F172A] relative flex overflow-x-hidden" style={{ fontFamily: 'var(--fonte-corpo)' }}>
            <div className="fixed top-0 left-1/4 w-96 h-96 rounded-full blur-[120px] -z-10" style={{ backgroundColor: `${corPrimaria}33` }}></div>
            <div className="fixed bottom-0 right-1/4 w-96 h-96 rounded-full blur-[120px] -z-10" style={{ backgroundColor: `${corSecundaria}33` }}></div>
                ...
                <div className="bg-white/5 backdrop-blur-xl border-white/10 rounded-2xl p-4 h-full flex-col shadow-2xl shadow-black/20">
                    <div className="flex items-center justify-between mb-8 px-1">
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-xl font-bold text-white whitespace-nowrap" style={{ fontFamily: 'var(--fonte-titulo)' }}>SIGE</h1>
                ...
                <header className="fixed top-0 right-0 left-0 lg:left-[260px] z-30 p-3 lg:p-6">
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl px-3 lg:px-6 py-3 flex items-center justify-between gap-2 shadow-lg shadow-black/10">
                        ...
                        <div className={`relative flex-1 transition-all duration-300 ${isSearchOpen? 'max-w-[500px] opacity-100' : 'max-w-0 opacity-0'} hidden md:block`}>
                            ...
                            <input ref={searchInputRef} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar no menu..." className="w-full pl-12 pr-4 py-3 bg-white/5 border-white/10 rounded-xl focus:outline-none text-sm text-white placeholder-gray-400" style={{ borderColor: isSearchOpen? corPrimaria : '' }} />
                        </div>
                        <div className="flex items-center gap-1.5 ml-auto">
                            <button onClick={() => window.innerWidth < 768? setIsSearchModalOpen(true) : setIsSearchOpen(!isSearchOpen)} className="p-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition flex-shrink-0"><Search className="w-5 h-5 text-white" /></button>
                            ...
                            <button className="hidden sm:flex items-center gap-2 p-2.5 lg:px-4 lg:py-3 bg-white/5 border-white/10 rounded-xl hover:bg-white/10 transition flex-shrink-0"><User className="w-5 h-5 text-white" /><span className="text-sm font-semibold text-white hidden lg:inline">{user.nome.split(' ')[0]}</span></button>
                            <button onClick={() => setLogoutOpen(true)} className="p-2.5 bg-red-500/10 border-red-500/20 rounded-xl hover:bg-red-500/20 hover:border-red-500/40 transition group flex-shrink-0" title="Sair"><Power className="w-5 h-5 text-red-400 group-hover:text-red-300 transition" /></button>
                        </div>
                    </div>
                </header>
                <main className="pt-28 lg:pt-32 p-3 lg:p-6 w-full"><Outlet /></main>
            </div>
            {isSearchModalOpen && (
                <div className="fixed inset-0 z-[60] bg-[#0F172A] flex-col p-4 md:hidden animate-in fade-in">
                    <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold text-white">Pesquisar</h2><button onClick={() => { setIsSearchModalOpen(false); setSearchQuery('') }} className="p-2 -mr-2"><X className="w-6 h-6 text-gray-300" /></button></div>
                    <div className="relative mb-4"><Search className="absolute left-4 top-3.5 w-5 h-5 text-gray-400" /><input ref={searchInputRef} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Pesquisar no menu..." className="w-full pl-12 pr-4 py-3.5 bg-white/5 border-white/10 rounded-xl focus:outline-none text-white placeholder-gray-400" style={{ borderColor: searchQuery? corPrimaria : '' }} /></div>
                    <div className="flex-1 overflow-y-auto bg-white/5 rounded-2xl border-white/10 p-2">
                        ...
                        {searchResults.map(item => (
                            <button key={item.id} onClick={() => handleNavigate(item.path)} className="w-full flex items-center gap-3 p-3 hover:bg-white/10 rounded-xl text-left transition">
                                ...
                                <div className="min-w-0"><p className="text-white font-medium truncate">{item.nome}</p><p className="text-xs text-gray-400">{item.type}</p></div>
                            </button>
                        ))}
                    </div>
                </div>
