...
export default function MainLayout() {
    ...
    const [tema, setTema] = useState<any>(null) // 👈 NOVO

    const searchInputRef = useRef<HTMLInputElement>(null)
    const navigate = useNavigate()
    const location = useLocation()

    // 👇 NOVO: Busca tema do banco ao montar
    useEffect(() => {
    ...
    useEffect(() => {
        const updateUser = () => {
            ...
            if (u) setUser(u)
            // Atualiza tema se mudar nas definições
    ...
    const corPrimaria = tema?.cor_primaria || '#3B82F6' // 👈 USA VARIAVEL
    ...
    return (
        <div className="min-h-screen w-full bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#0F172A] relative flex overflow-x-hidden">
            <div className="fixed top-0 left-1/4 w-96 h-96 rounded-full blur-[120px] -z-10" style={{ backgroundColor: `${corPrimaria}20` }}></div> {/* 👈 DINAMICO */}
                ...
                <div className="bg-white/5 backdrop-blur-xl border-white/10 rounded-2xl p-4 h-full flex-col shadow-2xl shadow-black/20">
                    <div className="flex items-center justify-between mb-8 px-1">
                        <div className="flex items-center gap-3">
                            <School className="w-8 h-8 flex-shrink-0" style={{ color: corPrimaria }} /> {/* 👈 DINAMICO */}
                    ...
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl px-3 lg:px-6 py-3 flex items-center justify-between gap-2 shadow-lg shadow-black/10">
                        ...
                        <div className={`relative flex-1 transition-all duration-300 ${isSearchOpen? 'max-w-[500px] opacity-100' : 'max-w-0 opacity-0'} hidden md:block`}>
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-3 bg-white/5 border-white/10 rounded-xl focus:outline-none text-sm text-white placeholder-gray-400"
                                style={{ borderColor: isSearchOpen? corPrimaria : '' }} // 👈 DINAMICO
                            />
                        </div>
                ...
                <div className="fixed inset-0 z-[60] bg-[#0F172A] flex-col p-4 md:hidden animate-in fade-in">
                    ...
                    <div className="relative mb-4">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3.5 bg-white/5 border-white/10 rounded-xl focus:outline-none text-white placeholder-gray-400"
                            style={{ borderColor: searchQuery? corPrimaria : '' }} // 👈 DINAMICO
                        />
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto bg-white/5 rounded-2xl border-white/10 p-2">
                        ...
                        {searching && <div className="p-4 flex justify-center"><Loader2 className="w-6 h-6 animate-spin" style={{ color: corPrimaria }} /></div>} {/* 👈 DINAMICO */}
                        ...
                        {searchResults.map(item => (
                            <button key={item.id} onClick={() => handleNavigate(item.path)} className="w-full flex items-center gap-3 p-3 hover:bg-white/10 rounded-xl text-left transition">
                                <item.Icon className="w-5 h-5 flex-shrink-0" style={{ color: corPrimaria }} /> {/* 👈 DINAMICO */}
