...
import {
    LayoutDashboard, Building2, Settings, LogOut, Search, Bell, Plus, ShieldCheck, Menu, X
} from 'lucide-react'
...
export default function MainLayout() {
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false) // <- AGORA VAI FUNCIONAR
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#0F172A] relative">
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 h-full flex flex-col">
                    ...
                    <nav className="space-y-1">
                        {menuItems.map(item => {
                            ...
                            return (
                                <button key={item.path} onClick={() => handleNavigate(item.path)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition ${isActive ? 'bg-white/10 text-white font-semibold border border-white/20' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
            ...
            <div className="lg:ml-[260px]">
                <header className="fixed top-0 right-0 left-0 lg:left-[260px] z-40 p-4 lg:p-8">
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl px-4 lg:px-6 py-3 lg:py-4 flex items-center justify-between">
                        ...
                        <div className="relative flex-1 max-w-[500px] hidden md:block">
                            ...
                            <input placeholder="Buscar escola..." className="w-full pl-12 pr-4 py-3 bg-white/5 border-white/10 rounded-xl focus:outline-none focus:border-[#3B82F6] text-sm text-white placeholder-gray-400" />
                        </div>
                        <div className="flex items-center gap-2 lg:gap-3">
                            <button className="p-2.5 lg:p-3 bg-white/5 border-white/10 rounded-xl hover:bg-white/10"><Bell className="w-5 h-5 text-white" /></button>
                            <button className="flex items-center gap-2 bg-gradient-to-r from-[#3B82F6] to-[#8B5CF6] text-white px-3 lg:px-5 py-2.5 lg:py-3 rounded-xl font-semibold text-xs lg:text-sm hover:shadow-lg hover:shadow-[#3B82F6]/30">
                ...
                <main className="pt-[88px] lg:pt-[112px] p-4 lg:p-8">
                    <Outlet />
                </main>
            </div>
        </div>
