...
export default function MainLayout() {
    ...
    return (
        ...
        >
            {/* SÓ mostra as bolas de blur no tema escuro */}
            {!isClaro && (
                <>
                    ...
                    <div className={`${cardClass} px-3 lg:px-5 py-2.5 flex items-center justify-between gap-2`}>
                        ...
                        <div className={`relative flex-1 transition-all duration-300 ${isSearchOpen? 'max-w-[500px] opacity-100' : 'max-w-0 opacity-0'} hidden md:block`}>
                            ...
                            <input
                                ...
                                className={`w-full pl-10 pr-4 py-2.5 ${bgCard} ${borderCard} rounded-xl focus:outline-none text-sm`}
                        ...
                        <div className="flex items-center gap-1.5 ml-auto">
                            ...
                            <button
                                onClick={toggleTema}
                                className={`p-2 ${bgCard} ${borderCard} rounded-xl ${hoverBg} transition flex-shrink-0`}
                                title={isClaro? 'Mudar para tema escuro' : 'Mudar para tema claro'}
                            >
                                {isClaro? <Moon className="w-4 h-4" style={{ color: textPrimary }} /> : <Sun className="w-4 h-4" style={{ color: textPrimary }} />}
                            </button>
            ...
            {isSearchModalOpen && (
                <div className="fixed inset-0 z-[60] flex flex-col p-4 md:hidden animate-in fade-in" style={{ backgroundColor: isClaro? 'var(--cor-fundo)' : '#0F172A' }}>
                    ...
                    <div className={`flex-1 overflow-y-auto ${cardClass} p-2`}>
                        {!searchQuery && <p className="text-center pt-10 text-sm" style={{ color: textSecondary }}>Digite para começar a pesquisar</p>}
                        ...
                        {searchResults.map(item => (
                            ...
                            <button key={item.id} onClick={() => handleNavigate(item.path)} className={`w-full flex items-center gap-3 p-2.5 ${hoverBg} rounded-xl text-left transition`}>
                                ...
                                <div className="min-w-0">
                                    <p className="font-medium truncate text-sm" style={{ color: textPrimary }}>{item.nome}</p>
